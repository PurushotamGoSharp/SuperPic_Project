...
static void CommonInitialization(MUKMediaThumbnailsViewController *viewController, UICollectionViewLayout *layout)
{
    ...
    viewController->_thumbnailCellSize = CGSizeMake((ScreenSize.width-5)/3, (ScreenSize.width-5)/3);
    
    viewController->_thumbnailCellSpacing = 1.0f;
...
{    
    if ([self.delegate respondsToSelector:@selector(thumbnailsViewController:carouselToPresentAfterSelectingItemAtIndex:)])
    {
        ...
        if (carouselViewController) {
            ...
            MUKMediaThumbnailsViewControllerToCarouselTransition transition = MUKMediaThumbnailsViewControllerToCarouselTransitionCoverVertical;
